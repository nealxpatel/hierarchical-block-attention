...
def run_all_gates(model, tok, cfg):
    ...
    ok = ok_e and ok_c and ok_p and ok_g
    ...
    log(f"[gates] equivalence={ok_e} causality={ok_c} path={ok_p} gradiso={ok_g} -> "
...
def check_training(cfg, planned_tps, steps, report):
    ...
    try:
        ...
        train(cfg, "stage1", resume=False, micro_batch=_mb, grad_accum=_ga,
              budget_s=1800, smoke=(DEVICE != "cuda"), shakedown=True)
    finally:
        heal.log = orig
...
def check_eval(cfg, report):
    """One PPL cell + one needle cell end-to-end (small)."""
    ...
    report["eval"] = det
    log(f"[shake:eval] {det}")


def run_shakedown(cfg, planned_tps=5000.0, steps=150, stage="all"):
    """Run the pre-flight shakedown and write REPORT. Returns the overall bool."""
    log(f"shakedown device={DEVICE} dtype={COMPUTE_DTYPE} planned_tps={planned_tps}")
    ...
    if stage in ("all", "train"):
        check_training(cfg, planned_tps, steps, report)
    if stage in ("all", "eval"):
        check_eval(cfg, report)
...
def main():
    ...
    args = ap.parse_args()
    cfg = HBAConfig()
    ok = run_shakedown(cfg, planned_tps=args.planned_tps, steps=args.steps, stage=args.stage)
